...
public List<EtapaTicket> obtenerEtapasPorFlujo(Integer idFlujo) {

    Flujo flujo = flujoRepository.findById(idFlujo)
            .orElseThrow(() -> new RuntimeException("Flujo no encontrado"));

    return flujo.getPasos().stream()
            .filter(paso -> Boolean.TRUE.equals(paso.getEstado()))
            .sorted((a, b) -> a.getOrden().compareTo(b.getOrden()))
            .map(paso -> {

                EtapaTicket dto = new EtapaTicket();

                dto.setIdPaso(paso.getIdPasosFlujo());
                dto.setDescripcion(paso.getDescripcion());
                dto.setEsActual(false); // lógica opcional
